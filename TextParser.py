                ...
                if self.words[1] != -1:  # PASSED ITEM THROUGH TO COMMAND
                    ...
                    if index_in_inventory == -1:
                        ...
                        if self.words[1] % 4 == 0:  # IF WEAPON
                            if player.weapon != 0:  # WEAPON ALREADY EQUIPPED
                                player.attack -= player.weapon * 5  # RETURN ATTACK STAT TO ORIGINAL VALUE
                                player.inventory.append(player.weapon)  # RETURN OLD WEAPON TO INVENTORY
                                player.weapon = self.words[1]  # EQUIP NEW WEAPON
                                player.attack += player.weapon * 5  # ALTER STATS FOR NEW WEAPON
                            else:
                                player.weapon = self.words[1]  # EQUIP
                                player.attack += player.weapon * 5  # ALTER STATS FOR NEW WEAPON
                        elif self.words[1] % 5 == 0:  # IF ARMOR
                            if player.armor != 0:  # ARMOR ALREADY EQUIPPED
                                player.defense -= player.armor * 5  # RETURN DEFENSE STAT TO ORIGINAL VALUE
                                player.inventory.append(player.armor)  # RETURN OLD ARMOR TO INVENTORY
                                player.armor = self.words[1]  # EQUIP NEW ARMOR
                                player.defense += player.armor * 5  # ALTER STATS FOR NEW ARMOR
                            else:
